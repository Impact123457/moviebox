...
import { CommentCardType } from "@/app/components/CommentCard"; 
...
export default async function MoviePage(props: { params: Promise<{ id: string }> }) {
  ...
  const comments = await client.fetch(COMMENT_QUERY, {id})
  ...
  {movie ? (
    <>
      ...
      <div className="p-5 my-5 shadow-lg md:w-[900px] mx-auto">
        <CommentHeader _id={movie._id}/>
          <ul className="comment-grid">
            {comments?.length > 0 ? (
              comments.map((post: CommentCardType) => (
